...
def list_prodi(db: Session = Depends(get_db), user_info: dict = Depends(verify_token)):
...
def list_fakultas(db: Session = Depends(get_db), user_info: dict = Depends(verify_token)):
...
def get_fakultas_by_id(id: str, db: Session = Depends(get_db), user_info: dict = Depends(verify_token)):
...
def register_user(user_data: UserAuth, db: Session = Depends(get_db)): # PROTEKSI DIHAPUS DARI SINI
    try:
...
def login_user(user_data: UserAuth, response: Response, db: Session = Depends(get_db)): # PROTEKSI DIHAPUS DARI SINI
...
def refresh_access_token(request: Request, response: Response, db: Session = Depends(get_db)): # PROTEKSI DIHAPUS DARI SINI
